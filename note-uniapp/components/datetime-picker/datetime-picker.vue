<template>
	<view class="datatime">
		<picker ref="datetimePicker" mode="multiSelector" :range="range" range-key="text" @change="change" @columnchange="columnchange" :value="value" :disabled="disabled">
...
export default {
	...
	methods: {
		...
		setDateStr(date) {
			...
			if(this.fields == 'hour') {
				this.dateStr = `${dt.YYYY}-${dt.M}-${dt.D} ${dt.h}时`;
				return;
			}
