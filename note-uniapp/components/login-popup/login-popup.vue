		...
		<view>
			<view class="pl-2 mb-5">
				<view class="flex just-content-between align-center">
					...
					<view class="u-avatar" @click="openUser" :hover-class="isLogin?'':'bg-hover-light'">
						<view :class="isLogin?'u-line-1':'p-1 border'" > 
			...
			<view class="n-p" v-for="(item,index) in list" :key="index" hover-class="bg-hover-light" @click="onClick(item)">
				<template v-if="item.isTemporary">
					<view class="mr-2 ml-2">
						<u-icon :name="item.icon" size="22" color="#6c757d"></u-icon>
					</view>
					<view class="p-right">
						<view class="font-md text-secondary">{{item.name}}</view>
					</view>
				</template>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openUser(){
				if(this.isLogin){
					uni.$u.route('/pages/user/details')
					return;
				}else{
