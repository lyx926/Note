<template>
	<view class="w-100">
		...
		<!-- 浮动按钮 -->
		<u-icon class="plus-button" @click="save()" name="plus" color="#ffffff" size="11"></u-icon>

		<view class="flex justify-center">
			<u--form :model="dataInfo" ref="uForm" labelWidth="auto" style="width: 95%;">
				...
				<template v-if="3==dataInfo.type">
					...
					<u-form-item label="是否还完 :" prop="isPayOff" borderBottom>
						<u-radio-group v-model="dataInfo.isPayOff">
							<u-radio :customStyle="{marginRight: '16px'}" v-for="(item, index) in isPayOffList"
				...
				<u-form-item label="支付方式 :" prop="payType" borderBottom required>
					<u-radio-group v-model="dataInfo.payType" style="width: 300rpx;" class="flex-wrap align-center">
						<u-radio :customStyle="{marginRight: '16px',marginBottom: '2px'}"
							v-for="(item, index) in payList" :key="index" :label="item.val" :name="item.key">
						</u-radio>
					</u-radio-group>
				</u-form-item>
				<u-form-item label="记录时间 :" prop="updateTime" borderBottom required>
					<datetime-picker ref="datetimePicker" fields="day" start="2022-01-00 00:00:00"
						end="2125-12-12 24:59:59" :defaultValue="dataInfo.updateTime" @change="saveTime"
						placeholder="请选择时间"></datetime-picker>
				</u-form-item>
				<u-form-item>
					<male-editor :uploadFile="upFile" placeholder="描述" :html="dataInfo.details"
						:fromItemHeight="3==dataInfo.type?410:300"></male-editor>
				</u-form-item>
			</u--form>
		</view>
	</view>
