<template>
	<view>
		<my-paging ref="paging" @query="queryList" @onRefresh="onRefresh">
			<view slot="top">
				<!-- 自定义导航栏 -->
				<tickler-navbar title="账单" rightIcon="plus" @leftClick="loginMenuShow=true" @rightClick="toSavePage">
				</tickler-navbar>

				<!-- 浮动按钮 -->
				<u-icon class="plus-button" @click="toSavePage" name="plus" color="#ffffff" size="11"></u-icon>

				<!-- tab -->
				<v-tabs v-model="tabIndex" height="90rpx" :scroll="false" fontSize="30rpx" :tabs="listDataTab"
		...
		<term-picker :visable.sync="pickerVisable" timeLimit themeColor="#10BE9D" @confirm="confirmPicker">
		</term-picker>
	...
	export default {
		...
		methods: {
			...
			clickBtn(index, id) {
				...
				if (0 == index) {
					//查看
					uni.$u.route('/pages/bill/details', {
						id: id
					})
				} else if (1 == index) {
					//添加
					uni.$u.route('/pages/bill/add', {
						pId: id
					})
				} else if (2 == index) {
					//编辑
					uni.$u.route('/pages/bill/update', {
						id: id
					})
				} else {
					deleteData({
