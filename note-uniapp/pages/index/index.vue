<template>
	<view>
		<tickler-navbar title="记录" rightIcon="plus" @leftClick="loginMenuShow=true" @rightClick="openListShow=true">
		</tickler-navbar>

		<!-- 浮动按钮 -->
		<u-icon class="plus-button" @click="openListShow=true" name="plus" color="#ffffff" size="11"></u-icon>
		...
		<view class="mx-2">
			<u--form :model="form">
				...
				<u-form-item prop="createTime" label="时间" borderBottom>
					<datetime-picker ref="datetimePicker" fields="hour" start="2022-01-00 00" end="2125-12-12 24"
						:defaultValue="form.createTime" @change="saveTime" placeholder="请选择时间"></datetime-picker>
				</u-form-item>
				<u-form-item prop="details">
					<male-editor :uploadFile="upFile" :html="form.details" :fromItemHeight="125" @input="editorChange">
					</male-editor>
				</u-form-item>
			</u--form>
		</view>
		...
		<u-popup :show="openListShow" safeAreaInsetTop mode="right" @close="openListShow=false"
			...
			<my-paging ref="paging" v-model="dataList" @query="queryList" style="position:absolute;padding-top: 8rpx;">
				<u-swipe-action>
					<u-swipe-action-item :show="item.show" :index="index" :name="index" @click="click"
						:options="options" v-for="(item, index) in dataList" :key="item.id" class="border-bottom">
						<view class="ml-1">
			...
			<view style="position: fixed;bottom: 0;width: 500rpx">
				<u-button @click="popupAdd" text="添加"></u-button>
			</view>
		</u-popup>
		...
		methods: {
			...
			saveData() {
				if (!this.$store.state.isTemporary) {
					//保存逻辑
					this.form.details = this.details == "" ? this.form.details : this.details;
