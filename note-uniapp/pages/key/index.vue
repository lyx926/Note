<template>
	<view class="w-100">
		<my-paging ref="paging" v-model="dataList" @query="queryList">
			<view slot="top">
				<tickler-navbar title="账号" rightIcon="plus" @leftClick="loginMenuShow=true" @rightClick="clickAdd">
				</tickler-navbar>
			</view>

			<!-- 浮动按钮 -->
			<u-icon class="plus-button" @click="clickAdd" name="plus" color="#ffffff" size="11"></u-icon>

			<!-- 数据 -->
			<u-swipe-action>
				<u-swipe-action-item :show="item.show" :index="index" :name="index" @click="clickSwipe"
					:options="options" v-for="(item, index) in dataList" :key="item.id">
		...
		<!-- 添加 -->
		<u-popup :show="saveShow" mode="center" :closeOnClickOverlay="false" :round="14" closeable
			@close="saveShow=false" :customStyle='{"width":"95%"}'>
			<view class="px-3 py-1">
				<u--form :model="upModel" ref="upForm" labelWidth="auto" :labelStyle='{"width":"100%"}'>
					<u-form-item label="账号 :" leftIcon="account">
						<u--input border="bottom" v-model="upModel.account" focus placeholder="请输入账号" />
					</u-form-item>
					<u-form-item label="密码 :" prop="password" required>
						<input-box ref="password" v-model="upModel.password" type="password"
							:inputValue="upModel.password" :verification="['isNull']" :verificationTip="['密码不能为空','']"
							placeholder="请输入密码" />
					</u-form-item>
					...
					<u-form-item label="简介 :" prop="desc" required>
						<u--textarea border="bottom" maxlength="120" autoHeight v-model="upModel.desc"
							placeholder="请填写简介"></u--textarea>
					</u-form-item>
				</u--form>
