<template>
	<view class="login">
			...
			<view class="main">
				...
				<block v-if="tabCurrentIndex === 2">
					<u--form :model="forgetParams" ref="forget" style="width: 80%;margin: 0 auto;" labelWidth="65">
						<u-form-item label="手机号码" prop="mobile">
							<u--input type="number" placeholder="请填写手机号码"
						...
						<u-form-item label="密码" prop="password">
							<input-box ref="password" v-model="forgetParams.password" type="password"
							  :verification="['isNull']" :verificationTip="['新密码不能为空','']" placeholder="请填写新密码"/>
						</u-form-item>
						<u-form-item label="验证码" prop="code" >
							<u--input fontSize="16" v-model="forgetParams.code"
								border="bottom" placeholder="请填写验证码"></u--input>
							<u-button slot="right" @tap="getCode" plain
								 type="success" shape="circle"
								 :disabled="readonly">{{codeText}}</u-button>
						</u-form-item>
					</u--form>
		...
		<!-- 发送验证码 -->	
		<u-code ref="uCode" @change="codeChange" keep-running
			@start="readonly = true" @end="readonly = false" ></u-code>
	</view>
</template>
<script>
	...
	export default {
		data() {
			return {
				...
				loginTypeList: [
					{ text: '临登', icon: '/static/icon/temporary_login.png' },
					{ text: '微信', icon: '/static/icon/wechat.png' },
					{ text: 'QQ', icon: '/static/icon/QQ.png' }
				],
				rules:{
					nickname:{
							required: true,
		...
		methods: {
			...
			getCode() {
				if (!this.forgetParams.mobile||!uni.$u.test.mobile(this.forgetParams.mobile)) {
